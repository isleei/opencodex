...
export const zh: Record<TKey, string> = {
  ...
  "dash.injectionHint": "选择供下方两个控件共用的模型和可选推理强度。",
  "dash.syncCodexSubagentDefaults": "用作原生 Codex 子代理默认值",
  "dash.syncCodexSubagentDefaultsHint": "默认关闭。当 OpenCodex 管理 Codex 路由时，同步或重启会将所选模型和推理强度应用为新 Codex 任务的原生 Codex [agents] 默认值。此设置本身不会触发委托；现有的用户自有 [agents] 默认值会保留而不会被覆盖。",
  "dash.multiAgentGuidance": "OpenCodex 多代理指引",
  "dash.multiAgentGuidanceHint": "添加由 OpenCodex 编写的委托指引。它与上方的原生 Codex 默认值相互独立，不会更改 v1/v2 界面、子代理清单、路由或 effort 上限。",
  ...
  "codexAuth.autoSwitchDesc": "使用率达到或超过 {threshold}% 且存在使用率更低的可用账号时切换；Go/Free 仅使用 30 天窗口",
