...
export const ru: Record<TKey, string> = {
  ...
  "dash.injectionHint": "Выберите модель и необязательный уровень рассуждений, общие для двух настроек ниже.",
  "dash.syncCodexSubagentDefaults": "Использовать как нативные значения подагентов Codex по умолчанию",
  "dash.syncCodexSubagentDefaultsHint": "По умолчанию выключено. Если маршрутизацией Codex управляет OpenCodex, синхронизация или перезапуск применяет выбранные модель и уровень рассуждений как нативные значения Codex [agents] для новых задач. Эта настройка сама не вызывает делегирование; существующие пользовательские значения [agents] сохраняются и не перезаписываются.",
  "dash.multiAgentGuidance": "Мультиагентное руководство OpenCodex",
  "dash.multiAgentGuidanceHint": "Добавляет инструкции делегирования от OpenCodex. Это отдельная настройка от нативных значений Codex выше; она не меняет поверхность v1/v2, список подагентов, маршрутизацию и пределы effort.",
  ...
  "codexAuth.autoSwitchDesc": "Переключает при использовании {threshold}% или выше, если доступен аккаунт с меньшим использованием; тарифы Go/Free используют только окно 30 дней",
