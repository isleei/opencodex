...
export const ja: Record<TKey, string> = {
  ...
  "dash.injectionHint": "下の 2 つの設定で共用するモデルと任意の推論負荷を選択します。",
  "dash.syncCodexSubagentDefaults": "ネイティブ Codex サブエージェントの既定値として使用",
  "dash.syncCodexSubagentDefaultsHint": "既定ではオフです。OpenCodex が Codex ルーティングを管理している場合、同期または再起動によって選択したモデルと推論負荷を新しい Codex タスクのネイティブ Codex [agents] 既定値として適用します。この設定自体は委任を発生させず、既存のユーザー所有 [agents] 既定値は上書きせずに保持します。",
  "dash.multiAgentGuidance": "OpenCodex マルチエージェントガイダンス",
  "dash.multiAgentGuidanceHint": "OpenCodex が作成する委任指示を追加します。上のネイティブ Codex 既定値とは別で、v1/v2 サーフェス、サブエージェントロスター、ルーティング、effort 上限は変わりません。",
