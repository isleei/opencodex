...
function resolveMatchedPriceInner(
  ...
  if (provider === "google-antigravity-native" || provider.startsWith("google-antigravity-native")) {
    const baseMatch = resolveMatchedPriceInner("google-antigravity", modelId, overlays, userOverlays, options);
    if (baseMatch) return { ...baseMatch, provider };
  }
  if (provider === "xai-native" || provider.startsWith("xai-native")) {
    const baseMatch = resolveMatchedPriceInner("xai", modelId, overlays, userOverlays, options)
      ?? (modelId.endsWith("-build") ? resolveMatchedPriceInner("xai", modelId.slice(0, -"-build".length), overlays, userOverlays, options) : null);
    if (baseMatch) return { ...baseMatch, provider };
  }
