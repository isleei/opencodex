...
export function inferXaiPlan(
  ...
  // Prefer "Grok Free" so GUI Codex plan filters do not strip weekly bars.
  return "Grok Free";
}

/** Merge monthly billing + credits-format weekly billing (CPA / CLIProxyAPI ecosystem). */
...
export async function fetchXaiBillingUsage(accessToken: string): Promise<XaiAccountUsage | null> {
  const [monthlyBilling, creditsBilling, user] = await Promise.all([
    fetchXaiJson(XAI_BILLING_URL, accessToken),
    fetchXaiJson(XAI_CREDITS_URL, accessToken),
