...
import { ACCOUNT_QUOTA_DISK_MAX_AGE_MS, cancelPendingAccountQuotaPersist, readPersistedAccountQuotas, schedulePersistAccountQuotas } from "../account-quota-disk";
...
export function sweepExpiredProviderAccountQuotaRows(now = Date.now()): number {
  let removed = 0;
  for (const [key, entry] of accountQuotaCache) {
    // Subscription-group AGY catalogs stay eligible for the management SWR surface for
    // their full disk retention window; the probe TTL alone must not drop them.
