...
async function readExplicitAccountQuota(provider: string, accountId: string, configured?: OcxProviderConfig): Promise<{
  ...
  isCurrent: () => boolean;
  plan?: string;
  accountQuota?: ProviderQuota | null;
} | null> {
  const target = explicitQuotaConfig(provider, configured);
  ...
  let accountQuota: ProviderQuota | null | undefined;
  switch (provider) {
    case "xai": {
      result = await fetchXaiQuota(provider, { accessToken, upstreamAccountId: credential.accountId });
      const userId = credential.accountId?.trim() || xaiUserIdFromAccessToken(accessToken);
      if (userId) {
        ...
        if (usage) {
          ...
          if (usage.quota) {
            accountQuota = {
              ...usage.quota,
              ...(result && typeof result !== "symbol" && result.quota.weeklyPercent !== undefined ? { weeklyPercent: result.quota.weeklyPercent } : {}),
              ...(result && typeof result !== "symbol" && result.quota.weeklyResetAt !== undefined ? { weeklyResetAt: result.quota.weeklyResetAt } : {}),
            };
          }
        }
      }
  ...
  return { result, identity, isCurrent, ...(plan ? { plan } : {}), ...(accountQuota !== undefined ? { accountQuota } : {}) };
}
...
async function fetchExplicitAccountQuota(provider: string, accountId: string, force: boolean, configured?: OcxProviderConfig): Promise<AccountQuotaCacheEntry> {
  ...
  const flight = (async (): Promise<AccountQuotaCacheEntry> => {
    ...
    const quota = current && result && typeof result !== "symbol" ? (read?.accountQuota ?? result.quota) : null;
    ...
    const entry: AccountQuotaCacheEntry = {
      ...
      ...(read?.plan ? { plan: read.plan } : cached?.plan ? { plan: cached.plan } : {}),
...
async function fetchAccountQuota(
  ...
  if (provider === "google-antigravity" || provider === "anthropic") hydrateAccountQuotaCache();
  ...
  const probe = (async (): Promise<AccountQuotaCacheEntry> => {
    try {
      ...
      const entry: AccountQuotaCacheEntry = {
        ts: Date.now(),
        quota: provider === "anthropic" ? normalizeAnthropicQuota(quota, Date.now()) : quota,
...
function classifyAntigravityFamily(
  modelId: string,
  modelInfo: Record<string, unknown>,
  quotaInfo: Record<string, unknown>,
): "Gem" | "Cla" | null {
  const normalizedModelId = modelId.toLowerCase();
  const displayName = typeof modelInfo.displayName === "string" ? modelInfo.displayName.toLowerCase() : "";
  const tier = typeof quotaInfo.tier === "string" ? quotaInfo.tier.toLowerCase() : "";
  const match = (pattern: string) =>
    normalizedModelId.includes(pattern) || displayName.includes(pattern) || tier.includes(pattern);

  if (match("claude")) return "Cla";
  if (match("gemini")) return "Gem";
  return null;
}
...
function antigravityUsedPercent(quotaInfo: Record<string, unknown>): number | undefined {
  ...
  const rawFraction = toFiniteNumber(target.remainingFraction);
  if (rawFraction !== undefined) {
    if (rawFraction < 0 || rawFraction > 1) return undefined;
    return normalizePercent(100 - rawFraction * 100);
  }
  const rawPercentage = toFiniteNumber(target.remainingPercentage);
  if (rawPercentage !== undefined) {
    if (rawPercentage < 0 || rawPercentage > 100) return undefined;
    return normalizePercent(100 - rawPercentage);
  }
  return undefined;
}
...
function parseAntigravityQuotaSummary(body: Record<string, unknown> | null): ProviderQuota | null {
  ...
  for (const rawGroup of groups) {
    ...
    const isGemini = group.groupId === "gemini" || groupName.includes("gemini")
      || buckets.some(b => {
        const id = asRecord(b)?.bucketId;
        return typeof id === "string" && id.startsWith("gemini");
      });
    const isClaude = group.groupId === "3p" || groupName.includes("claude") || groupName.includes("3p") || groupName.includes("gpt")
      || buckets.some(b => {
        const id = asRecord(b)?.bucketId;
        return typeof id === "string" && id.startsWith("3p");
      });
  ...
  const subQuota = antigravitySubscriptionQuota(body);
  return {
    ...(subQuota?.agyQuotaGroups ? { agyQuotaGroups: subQuota.agyQuotaGroups } : {}),
    customWindows,
    updatedAt: Date.now(),
  };
}
