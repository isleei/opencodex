    ...
    buildRequest(parsed: OcxParsedRequest, incoming: IncomingMeta) {
      ...
      const normalizedBody =
        provider.modelSuffixBracketStrip
          ...
          : unnormalizedBody;
      // Fork: clamp an operator ceiling onto the model-normalized body so the cap is
      // applied to the id the upstream actually receives.
      const finalBody = clampResponsesMaxOutputTokens(normalizedBody, provider, parsed.modelId);
      if (isCanonicalOpenAiForwardProvider(provider)) {
