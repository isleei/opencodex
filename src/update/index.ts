...
export function latestVersion(
  ...
  const cacheKey = `${tag}:${installer}:${owner ?? ""}`;
  ...
  if (installer === "pnpm" && !resolvedOwner) {
    latestVersionCache.set(cacheKey, { value: null, expiresAt: now + LATEST_VERSION_TTL_MS });
    return null;
  }
  ...
  const value = r.status === 0 && typeof r.stdout === "string" ? (r.stdout.trim() || null) : null;
  latestVersionCache.set(cacheKey, { value, expiresAt: now + LATEST_VERSION_TTL_MS });
  return value;
}
