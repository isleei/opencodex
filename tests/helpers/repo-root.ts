...
const PACKAGE_NAMES = new Set(["@bitkyc08/opencodex", "@iislee/opencodex"]);
...
export function repoRoot(): string {
  ...
  for (let hops = 0; hops < 8; hops += 1) {
    ...
    if (existsSync(candidate)) {
      ...
      if (typeof parsed.name === "string" && (PACKAGE_NAMES.has(parsed.name) || parsed.name.endsWith("/opencodex"))) {
        cached = dir;
        return dir;
      }
    }
